...
SELECT ev.id                                      AS id,
       ...
       SUM(CASE WHEN participant.need_catering THEN 1 ELSE 0 END)      AS count_catering,
       SUM(CASE WHEN participant.need_accommodation THEN 1 ELSE 0 END) AS count_accommodation,
...
GROUP BY ev.id,
         ...
         tentative.count_tentative,
         pending.count_pending,
         soprano.count_soprano,
         alto.count_alto,
         tenor.count_tenor,
         bass.count_bass,
         tentative.tentative_name,
         soprano.soprano_name,
         alto.alto_name,
         tenor.tenor_name,
         bass.bass_name,
         declined.declined_name
